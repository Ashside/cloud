...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default=env_or_default("SPO_SAVE_DIR", "../out"), help="模型保存目录")
    parser.add_argument('--save_weight', default=env_or_default("SPO_SAVE_WEIGHT", 'spo'), type=str, help="保存权重的前缀名")
    parser.add_argument("--epochs", type=int, default=env_or_default("SPO_EPOCHS", 1, int), help="训练轮数")
    parser.add_argument("--batch_size", type=int, default=env_or_default("SPO_BATCH_SIZE", 2, int), help="batch size")
    parser.add_argument("--learning_rate", type=float, default=env_or_default("SPO_LEARNING_RATE", 1e-7, float), help="初始学习率")
    parser.add_argument("--device", type=str, default=env_or_default("SPO_DEVICE", "cuda:0" if torch.cuda.is_available() else "cpu"), help="训练设备")
    parser.add_argument("--dtype", type=str, default=env_or_default("SPO_DTYPE", "bfloat16"), help="混合精度类型")
    parser.add_argument("--num_workers", type=int, default=env_or_default("SPO_NUM_WORKERS", 1, int), help="数据加载线程数")
    parser.add_argument("--accumulation_steps", type=int, default=env_or_default("SPO_ACCUMULATION_STEPS", 4, int), help="梯度累积步数")
    parser.add_argument("--grad_clip", type=float, default=env_or_default("SPO_GRAD_CLIP", 1.0, float), help="梯度裁剪阈值")
    parser.add_argument("--log_interval", type=int, default=env_or_default("SPO_LOG_INTERVAL", 1, int), help="日志打印间隔")
    parser.add_argument("--save_interval", type=int, default=env_or_default("SPO_SAVE_INTERVAL", 10, int), help="模型保存间隔")
    parser.add_argument('--hidden_size', default=env_or_default("SPO_HIDDEN_SIZE", 512, int), type=int, help="隐藏层维度")
    parser.add_argument('--num_hidden_layers', default=env_or_default("SPO_NUM_HIDDEN_LAYERS", 8, int), type=int, help="隐藏层数量")
    parser.add_argument('--use_moe', default=env_or_default("SPO_USE_MOE", 0, int), type=int, choices=[0, 1], help="是否使用MoE架构（0=否，1=是）")
    parser.add_argument('--max_seq_len', default=env_or_default("SPO_MAX_SEQ_LEN", 66, int), type=int, help="Prompt最大长度")
    parser.add_argument("--max_gen_len", type=int, default=env_or_default("SPO_MAX_GEN_LEN", 1536, int), help="生成的最大长度")
    parser.add_argument("--data_path", type=str, default=env_or_default("SPO_DATA_PATH", "../dataset/rlaif-mini.jsonl"), help="RLAIF数据路径")
    parser.add_argument("--beta", type=float, default=env_or_default("SPO_BETA", 0.02, float), help="KL惩罚系数")
    parser.add_argument("--reasoning", type=int, default=env_or_default("SPO_REASONING", 1, int), choices=[0, 1], help='推理模型类型（0=普通模型，1=推理模型）')
    parser.add_argument("--reward_model_path", type=str, default=env_or_default("SPO_REWARD_MODEL_PATH", "../../internlm2-1_8b-reward"), help="Reward模型路径")
    parser.add_argument('--from_resume', default=env_or_default("SPO_FROM_RESUME", 0, int), type=int, choices=[0, 1], help="是否自动检测&续训（0=否，1=是）")
    parser.add_argument("--use_wandb", action="store_true", default=env_bool("SPO_USE_WANDB", False), help="是否使用wandb")
    parser.add_argument("--wandb_project", type=str, default=env_or_default("SPO_WANDB_PROJECT", "MiniMind-SPO"), help="wandb项目名")
